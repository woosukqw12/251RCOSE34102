...
evaluation_result PreemptivePriority(process *p, int len)
{
	//init
	int TotalTime = 0;
	...
	while(1){
		...
		for (int idx=NoP_in_JQ-1; idx>=0; idx--){
			if (job_queue[idx].arrival_time <= TotalTime){
				insert_ready_queue(ready_queue, pop_from_job_queue(job_queue, idx), 2);
			}
		}
		...
		if (NoP_in_WQ > 0){
			if (wait_queue[0].IO_burst_time == 0){
				...
				insert_ready_queue(ready_queue, pop_from_wait_queue(wait_queue, 0), 2);
